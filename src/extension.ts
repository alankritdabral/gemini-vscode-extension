...
export function activate(context: vscode.ExtensionContext) {
  try {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        "gemini-cli-ui.resumeSessionFromTree",
        (sessionId: string) => {
...
class GeminiChatViewProvider implements vscode.WebviewViewProvider {
  ...
  public async getSessions(): Promise<any[]> {
    ...
    const cwd =
      vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || fallbackCwd;
    ...
    return new Promise((resolve) => {
      ...
      child.on("error", (err) => {
        this._outputChannel.appendLine(
          `[Error] Failed to spawn gemini for sessions: ${err.message}`,
        );
        resolve([]);
      });
    });
...
class GeminiSessionsProvider implements vscode.TreeDataProvider<SessionItem> {
  ...
  async getChildren(element?: SessionItem): Promise<SessionItem[]> {
    ...
    this.outputChannel.appendLine(
      `[Extension] Loading session tree from ${nodePath} ${geminiPath} in ${cwd}${cwd === fallbackCwd ? " (fallback)" : ""}`,
    );
    ...
    return new Promise((resolve) => {
      ...
      child.on("error", (err: any) => {
        ...
        if (err.code === "ENOENT") {
          this.outputChannel.appendLine(
            `[Error] Gemini CLI not found at '${geminiPath}'. Please check VS Code settings.`,
          );
          vscode.window.showErrorMessage(
